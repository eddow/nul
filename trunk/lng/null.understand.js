...
nul.understanding = {
	...
	emptyBase: function(prntUb) {
		return { 
			...
			{
				if(!stringed) this.build.defined(name);
	...
	expression: function(xpr, ub) {
		...
		case '?':
			...
			return ub.build.and3([nul.build().assert(guard), value]);
		case ',..':
			var lst;
			if(','== xpr.operands[0].operator) lst = xpr.operands[0].operands;
			else lst = [xpr.operands[0]];
			var ops = nul.understanding.understandOperands(lst, ub);
			ops.follow = nul.understanding.understand(xpr.operands[1], ub);
			return ub.build.list(ops);
		}
		var ops = nul.understanding.understandOperands(xpr.operands, ub);
		if(['&&', '||', '+' ,'-' ,'*' ,'/' ,'%' ,'&' ,'|' ,'^' ,'&&' ,'||'].contains(xpr.operator))
			return ub.build.cumulExpr(xpr.operator, ops);
		if(['<','>','<=','>='].contains(xpr.operator))
			return ub.build.biExpr(xpr.operator, ops);
		switch(xpr.operator)
		{
			case ':-':	return ub.build.lambda(ops[0], ops[1]);
			case ',':	return ub.build.list(ops);
			case '=':	return ub.build.unification(ops);
			case ';':	return ub.build.and3(ops);
			case '[]':	return ub.build.or3(ops);
			case ':':	return ub.build.xor3(ops);
			case '<<=':	return ub.build.seAppend(ops[0], ops[1]);
	...
	preceded: function(prc, ub) {
		...
		if(prc.operator == '?') return ub.build.assert(op);
		if(prc.operator == ',') return ub.build.list([op]);
		return ub.build.preceded(prc.operator,op);
	},
	postceded: function(pstc, ub) {
		var op = nul.understanding.understand(pstc.operand, ub);
		if(pstc.operator == '!') return ub.build.extraction(op);
	...
	atom: function(atm, ub) {
		...
		switch(atm.type)
		{
			...
			case "number":
				...
				break;
			case "alphanum" :
				try { return ub.resolve(atm.value); }
				catch(err) {
					if(nul.understanding.unresolvable!= err) throw err;
					if('_'== atm.value) return nul.understanding.definition(
						nul.compiled.definition('_',
							nul.compiled.atom({type:'alphanum', value:'_'}), atm.declared ),
						ub);
					throw nul.semanticException('Identifier "'+atm.value+'" undefined.');
				}
				break;
			default:
				throw nul.semanticException('unknown atom: ' + atm.type + ' - ' + atm.value);
		}
		return ub.build.atom(value);
	},
	xml: function(o, ub) {
		var attrs = {}, content = [];
		...
		return ub.build.xml( o.node, attrs, content );
	},
	application: function(o, ub) {
		return ub.build.application(
			nul.understanding.understand(o.item, ub),
			nul.understanding.understand(o.applied, ub) );
	},
	set: function(o, ub) {
		return ub.build.set(o.content?nul.understanding.understand(o.content, ub):null);
	},
	...
	definition: function(ctxtl, ub, locals) {
		if(ctxtl.type) {
			...
			var tp = nul.understanding.understand(ctxtl.type, sub);
			var lcl = nul.build().local(2, lindx, ctxtl.decl);
			
			return ub.build.and3([
				nul.build().application(tp, lcl),
				val]);
		} else {
			ub.createFreedom(ctxtl.decl);
			return nul.understanding.understand(ctxtl.value, ub, 'noub');
		}
	},
	...
	prototype: function(o, ub) {
		...
		var rv = ub.build.prototype(applied, o.name, nul.understanding.understand(o.value, ub));
		ub.createFreedom(o.name, null, true)
		return rv;
	},
	objectivity: function(o, ub) {
		//TODO: a->b  devient \/z a::b z. ; z
		...
		return ub.build.objectivity(applied, value);
	}
};